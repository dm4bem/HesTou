...
@author: Ibrahim Touré, Moritz Hessler
   based on code of
       @author: cghiaus
...
ending_time = '18:00:00'  # hh:mm:ss time for ending weather simulations
...
def degree_hours (temp_inside_day, temp_inside_night):
    ...
    period_start = weather_data_year + '-01-01'
    period_end = weather_data_year + '-12-31'
...
q_DAE = G @ (-A @ θ + b)            # heat flows in steady state regarding DAE representation
print('temperature nodes and heat flows in steady state regarding DAE:')
print(f'θ = {θ} °C')
print(f"q_DAE = {q_DAE}, W")
...
q_SS = G @ (-A @ yss + b)           # heat flows in steady state regarding state-space representation
print('temperature nodes and heat flows in steady state regarding state-space:')
print(f'yss = {yss} °C')
print(f"q_SS = {q_SS}, W")
...
print('Begin of Dynamic Simulation:')
...
print(f'time step: dt = {dt} s = {dt / 60:.0f} min')
...
print('- step response: steady state value after step input, explicit Euler:')
print(f' {y_exp.loc["θ6"].tail(1).values[0]:.4f} °C')
print('- step response: steady state value after step input, implicit Euler:')
print(f' {y_imp.loc["θ6"].tail(1).values[0]:.4f} °C\n')
...
print('Behavior of controlled inside temperature can influence energy consumption')
print('Example: degree hours in one year')
print(f'Constant inside temperature ({T_set}°C) vs. different during day ({T_set_day}°C) and night ({T_set_night}):')
degree_hours(T_set_day, T_set_night)

print('\nTrace of the response to the weather:')
start_date = weather_data_year + starting_day + ' ' + starting_time
end_date = weather_data_year + ending_day + ' ' + ending_time
